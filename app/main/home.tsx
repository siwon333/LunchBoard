...
import { RoomEvent, PresenceStatus } from '../../types';
...
export default function HomeScreen() {
  ...
  function handlePresenceChange() {
    ...
    Alert.alert(
      ...
      undefined,
      statuses.map((s) => ({
        text: `${PRESENCE_CONFIG[s].emoji} ${PRESENCE_CONFIG[s].label}`,
        onPress: () => updatePresence(currentRoom.id, user.uid, s),
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white pt-14 pb-4 px-4 border-b border-gray-100 flex-row items-end justify-between">
        ...
        {myPresence && (
          <TouchableOpacity onPress={handlePresenceChange} className="items-center">
            <Text className="text-2xl">{PRESENCE_CONFIG[myPresence.status].emoji}</Text>
            <Text className="text-[10px] text-gray-400">{PRESENCE_CONFIG[myPresence.status].label}</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      >
        ...
        <Card className="mx-4 mb-4 p-3">
          ...
          {!myPresence && (
            <TouchableOpacity onPress={handlePresenceChange} className="mt-3 items-center">
              <Text className="text-xs text-sky-500 font-medium">내 상태 등록하기</Text>
            </TouchableOpacity>
          )}
        </Card>
